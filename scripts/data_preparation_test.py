...
# Set TEST_MODE to True to filter by one turbine
...
def load_data():
    ...
    # For testing purposes, filter to only one turbine if TEST_MODE is True
    if TEST_MODE:
        # Assuming 'Identifier (Plant)' uniquely identifies a turbine
        data = data[data['Identifier (Plant)'] == data['Identifier (Plant)'].unique()[0]]
        print(f"Test mode enabled: Using data from one turbine (ID: {data['Identifier (Plant)'].unique()[0]})")
